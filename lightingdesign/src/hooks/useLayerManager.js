...
export const useLayerManager = (initialLayers = null) => {
  ...
  const getVisibleSublayers = useCallback(
    (layerId) => {
  ...
  const filterProductsBySublayers = useCallback(
    (products, layerId) => {
      const layer = layers.find((l) => l.id === layerId);
        ...
        if (layer.id === layerId) {
          return {
            ...
            sublayers: [
              ...
              {
                id: newSublayerId,
                name: name || `Layer ${layer.sublayers.length + 1}`,
                visible: true,
                isDefault: false,
              },
            ],
        ...
        if (layer.id === layerId) {
          ...
          if (filteredSublayers.length === 0) {
            return {
              ...layer,
              sublayers: [
                { id: `${layerId}-default`, name: "Default", visible: true, isDefault: true },
              ],
        ...
        if (layer.id === layerId) {
          return {
            ...
            products: layer.products.map((product) =>
              productIds.includes(product.id) ? { ...product, sublayerId } : product,
            ),
          };
        }
        return layer;
